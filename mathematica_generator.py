...
class EquationGenerator:
    ...
    def get_exact_equations(self):
        ...
        def empty_dictionaries():
            ...
            for key in keys:
                decay_dictionary[key] = []
                create_dictionary[key] = []

        def get_creation_states(state0, resultant_states0):
            """ Appends the states to which the state0 decays for each process.

                :param state0: The state to be examined.

                :param resultant_states0: The list that contains the
                information of the state in the order

                [original_state, dictionary_of_final_states]
            """

            if self._get_state1_in_state2_indexes(state0, resultant_states0[0]):
                # If the state is a substate of the state, it will have a change to create the state.
                for key in keys:
                    # Look at each generated states by each process.
                    for state1 in resultant_states0[1][key]:
                        # If it can be generated by the current state, add it.
                        if self._get_state1_in_state2(state0, state1):
                            create_dictionary[key].append(resultant_states0[0])
                            break

        def get_decay_states(state0, resultant_states0):
            """ Appends the states to which the state0 decays for each process.
            ...
            # Determine if the state exists in the resultant state.
            if self._get_state1_in_state2(state0, resultant_states0[0]):
                # If the state is a substate of the state, it will have a change of decay.
                for key in keys:
                    decay_dictionary[key].extend(resultant_states0[1][key])

        def get_resulting_states(state0):
        ...
        for i, involved_state in enumerate(involved_states):
            ...
            resultant_states.append([involved_state, get_resulting_states(involved_state)])
        ...
        keys = resultant_states[0][1].keys()
        ...
        for i, low_state in enumerate(lowest_states):
            ...
            self.equations.append([low_state, cp.deepcopy(decay_dictionary), cp.deepcopy(create_dictionary)])
